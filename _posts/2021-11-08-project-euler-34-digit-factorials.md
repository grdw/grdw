...
It will always require >1 digit to make a number of a single digit. The only exception is 3 (which is not a sum). The upper bound I imagined would be 9_999_999 considering the length of that number will still remain 7 (the same as it starts out with). When doing 9! * 8 or (99_999_999 the length of its sum is lower than 8, and it will therefor never match). However letting the loop go till that far knowing it won't do much for the better part of the program, is a bit silly.
