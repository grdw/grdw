let sentences = [
       ...
       "When he removed his [noun-penis] from my [noun-anus], he was pleasantly surprised to see a [noun-faeces] staring back at him. He knew I couldn't wait to [verb-suck-root] the [noun-faeces] off his [noun-penis].",
...
let vocab = {
    ...
    "noun" : {
        ...
        "insertable" : [
            "a fifteen inch spiked vibrator",
            "a ten inch purple battery-operated monster",
            "a nine iron",
            "a barbie doll",
            "a gerbil",
            "a lightbulb",
...
function lookupVerb(path) {
    if (path.length < 3) {
        throw("Array too short");
    }

    let tense_map = { "root": 0, "past": 1, "present": 2 };
    let item = path[1];
    let tense_index = tense_map[path[2]];
    let group = vocab["verb"][item];
    return group[Math.floor(Math.random() * group.length)][tense_index];
};

function lookupNoun(path) {
    if (path.length < 2) {
        throw("Array too short");
    }

    let item = path[1];
    let group = vocab["noun"][item];
    return group[Math.floor(Math.random() * group.length)];
};


function lookupSimile(path) {
    if (path.length < 2) {
        throw("Array too short");
    }

    let item = path[1];
    let group = vocab["simile"][item];
    return group[Math.floor(Math.random() * group.length)];
};
...
function generateSentence() {
    ...
    var total = sentence.substr(0, startPos);
    var endPos = 0;

    while (startPos > -1) {
        endPos = sentence.indexOf(']', startPos);
        var marker = sentence.substring(startPos + 1, endPos);
        var markerPath = marker.split('-');

        switch(markerPath[0]) {
            case "verb":
                total += lookupVerb(markerPath);
                break;
            case "simile":
                total += lookupSimile(markerPath);
                break;
            case "noun":
                total += lookupNoun(markerPath);
                break;
            default:
                throw("No case")
                break;
        ...
        startPos = sentence.indexOf('[', startPos + 1);

        if (endPos <= (endPos + startPos - endPos)) {
            total += sentence.substring(
                endPos + 1,
    ...
    // Add the final bit
    total += sentence.substring(endPos + 1, sentence.length);
...
var voice = null;
var text = document.getElementById("fiftyshadestext");
var startButton = document.getElementById("fiftyshades-start");
var stopButton = document.getElementById("fiftyshades-stop");
var playing = false;
var utterance = null;
